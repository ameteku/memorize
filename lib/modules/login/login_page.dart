...
class LoginPage extends StatefulWidget {
  LoginPage({required this.appState, Key? key}) : super(key: key);
...
class _LoginPageState extends State<LoginPage> {
    ...
    return GestureDetector(
      onTap: () {
        if (userNameController.text != '' && passwordController.text != '') {
          UserRepo().getUser(username: userNameController.text, password: passwordController.text).then((user) {
            if (user == null) {
              showDialog(
                context: context,
                builder: (context) => AlertDialog(
                  title: Text('Wrong password/username, try again'),
                ),
              );
            } else {
              widget.appState.currentUser = user;
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
            ...
            gradient:
                LinearGradient(begin: Alignment.centerLeft, end: Alignment.centerRight, colors: [Color(0xfffbb448), Color(0xfff7892b)])),
        child: Text(
          'Login',
  ...
  Widget _createAccountLabel() {
    return InkWell(
      onTap: () async {
        bool isSet = await Navigator.push(
            context,
            MaterialPageRoute(
